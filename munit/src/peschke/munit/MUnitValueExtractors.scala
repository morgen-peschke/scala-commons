...
/** Provides instances so [[peschke.testing.ValueExtractor.Syntax]] can be used in munit tests
  */
trait MUnitValueExtractors  extends ValueExtractor.Syntax[Location] {
  ...
  implicit val failUsingMunitAssertions: Fail[Location] = new Fail[Location] {
    override def apply[A](msg: String, sourceInfo: Text[A], location: Location): Nothing =
      fail(s"${sourceInfo.source} $msg")(location)

    override def apply[A](msg: String, sourceInfo: Text[A], cause: Throwable, location: Location): Nothing =
      fail(s"${sourceInfo.source} $msg", cause)(location)
  }

  implicit val AtToLocation: At.To[Location] = at => new Location(at.path.value, at.line.value)
}
