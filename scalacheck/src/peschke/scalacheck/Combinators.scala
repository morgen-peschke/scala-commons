...
object Combinators {
   ...
   *
   * Note: to avoid running afoul of the invariants on [[Range]], elements
   * with index past [[Int.MaxValue]] will be dropped
   *
   * @param min Inclusive minimum for [[Range.start]]
   * @param max Inclusive maximum for [[Range.end]]
   */
  def inclusiveRanges(min: Int, max: Int): Gen[Range] =
    ...
    } yield (start to end by (if (start < end) step else -step)).take(Int.MaxValue)
   ...
   *
   * Note: to avoid running afoul of the invariants on [[Range]], the following normalization
   * is done:
   *  - If `max` is [[Int.MaxValue]], `max - 1` will be used instead.
   *    If `min` is [[Int.MaxValue]], it will also be lowered to avoid issues with [[Gen.chooseNum()]]
  ...
  def exclusiveRanges(min: Int, max: Int): Gen[Range] = {
    ...
    for {
      ...
      step <- Gen.chooseNum(1, (start - end).abs.max(1))
    } yield (start until end by (if (start < end) step else -step)).take(Int.MaxValue)
  }
   ...
   *
   * Note: to avoid running a foul of the invariants of [[Range]], normalization is applied.
   * See [[inclusiveRanges()]] and [[exclusiveRanges()]] for details.
   *
   * @param min Inclusive minimum for [[Range.start]]
   * @param max Inclusive maximum for [[Range.end]]
   */
   ...
   *
   * Values will honor [[Range.step]], so [[Range.contains]] should return `true` for all
