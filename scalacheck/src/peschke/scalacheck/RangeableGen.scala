...
object RangeableGen {
  ...
  implicit def liftToList[A]: RangeableGen[A, List[A]] =
    Combinators.listOfR(_, _)

  implicit def liftToVector[A]: RangeableGen[A, Vector[A]] =
    Combinators.vectorOfR(_, _)

  implicit def liftToChain[A]: RangeableGen[A, Chain[A]] =
    Combinators.chainOfR(_, _)
