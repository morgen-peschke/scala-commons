...
class RangeUtilsTest extends PropSpec {
  ...
  def calculationWasSafe[I](oldValue:   I, newValue: I, stepSize: I)
                           (implicit I: Integral[I])
    : Boolean = {
  ...
  def growthWasSafe(oldValue: NumericRange[Long], newValue: NumericRange[Long])
    : Boolean = {
    val stepWasNotChanged = oldValue.step eqv newValue.step
  ...
  property("RangeUtils.sliceNumeric should be equivalent to drop(f).take(u - f)") {
    forAll(stepsInNumericRange.flatMap(g => g._1.chooseSteps.map(g -> _))) {
      case ((range, steps1), steps2) =>
