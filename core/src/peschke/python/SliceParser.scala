...
object SliceParser {
  ...
  object ParseError {
    ...
    final case class StepCannotBeZero(index: Index, context: ErrorContext)                  extends ParseError
  ...
  def default[F[_]]
    (openBrace:   String, closeBrace: String, contextLength: Int)
    ...
    private val unbracedSliceParser: Parser0[Slice] = {
      ...
      def opt[A](parser: Parser[A], someContext: String, noneContext: String): Parser0[Option[A]] =
        Parser.oneOf0(
